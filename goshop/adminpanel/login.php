...
<head>
    ...
    <title>GoSHOP | Login Admin</title>
        ...
        <div class="login-box p-5 shadow">
            <form action="" method="post">
                <label for="" class="d-flex justify-content-center align-items-center mt-3"><strong>
                        <h2><img src="../assets/nav-bar item/shopping-cart.png" alt="" width="50px">ADMIN</h2>
                    </strong></label>
                <div>
                    <label for="username_admin_seller">Username</label>
                    <input type="text" class="form-control" name="username_admin_seller" id="username_admin_seller" autofocus autocomplete="off" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <a href="../userpanel/login.php" class="btn btn-primary d-flex justify-content-center align-items-center mt-3">Ganti Ke User</a>
                </div>
            </form>
        </div>
        <div class="mt-3">
            ...
            if (isset($_POST['loginbtn'])) {
                $username_admin_seller = htmlspecialchars($_POST['username_admin_seller']);
                $password = htmlspecialchars($_POST['password']);
                $query = mysqli_query($con, "SELECT * FROM admin_seller WHERE username_admin_seller='$username_admin_seller'");
                $countdata = mysqli_num_rows($query);
                $data = mysqli_fetch_array($query);
                if ($countdata > 0) {
                    if (password_verify($password, $data['password'])) {
                        $_SESSION['username_admin_seller'] = $data['username_admin_seller'];
                        $_SESSION['login'] = true;
